...
def nome_do_time(dados,id_numerica):
    x = dados['equipes'][id_numerica]['nome-comum']
    return x
...
def nome_campeao(dados):
    ...
    name_champ = dados['equipes'][id_Champ]['nome-comum']
    return name_champ
...
def qtos_libertadores(dados):
    #Retorna a quantidade de times classificados em String
    liberta = dados['fases']['2700']['faixas-classificacao']['classifica1']['faixa']
    minNum = liberta.split('-')[0]
    maxNum = liberta.split('-')[1]
    T = int(minNum)
    t = int(maxNum)
    qtdFinal = t - T  + 1
    return qtdFinal
...
def ids_dos_melhor_classificados(dados,numero_de_times):
    ...
    idMelhoresTimes = []
    i = 0 
    while numero_de_times > 0:
        Times = dados['fases']['2700']['classificacao']['grupo']['Único'][i]
        idMelhoresTimes.append(Times)
        i += 1
        numero_de_times -=1 
    return idMelhoresTimes
...
def classificados_libertadores(dados):
    ...
    idClassificados = ids_dos_melhor_classificados(dados, qtd)
...
def nomes_classificados_libertadores(dados):
    ...
    # qtd = 1
    # qtdClasssificados = qtos_libertadores(dados)
    nomesTimes = []
    for idTime in ids:
        time = dados['equipes'][idTime]['nome-comum']
        nomesTimes.append(time)
    return nomesTimes
...
def id_do_time(dados,nome_time):
    ...
    # idsTimes = []
    for idTime in ids:
        NomeTime = dados['equipes'][idTime]['nome-comum']
        if nome_time == NomeTime:
            return idTime
        else:
            if not NomeTime:
...
def dicionario_id_estadio_e_nro_jogos(dados):
    ...
    idEstadio = {}
    for jogo in jogos:
        estadio = jogos[jogo]['estadio_id']
        # if estadio != None:
        if not estadio in idEstadio:
            idEstadio[estadio] = 1
        else:
            idEstadio[estadio] += 1
    # dic = {x: id_estadios.count(x) for x in id_estadios}
    return idEstadio
...
#Com falha no segundo teste o len retorna um valor a maiss
def datas_de_jogo(dados):
    jogos = dados['fases']['2700']['jogos']['id']
    datas = []
    for jogo in jogos:
        data = jogos[jogo]['data']#recebe a data
...
def data_de_um_jogo(dados,id_jogo):
    data = dados=['fases']['2700']['jogos'][id_jogo]['data']
    return data
...
def busca_imprecisa_por_nome_de_time(dados,nome_time):
    ...
    for equipe in equipes:
        if nome_time in dados['equipes'][equipe]['nome-comum']:
            idsValidos.append(dados['equipes'][equipe]['nome-comum'])
        else:
            if nome_time in dados['equipes'][equipe]['nome-slug']:
                idsValidos.append(dados['equipes'][equipe]['nome-slug'])
            else:
                if nome_time in dados['equipes'][equipe]['sigla']:
                    idsValidos.append(dados['equipes'][equipe]['sigla'])
                else:
                    if nome_time in dados['equipes'][equipe]['nome']:
                        idsValidos.append(dados['equipes'][equipe]['nome'])
